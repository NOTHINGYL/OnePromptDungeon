import type { ItemKind, MonsterKind, ShopUpgrade } from "./types/game";

export type Language = "en" | "zh";

export type TranslationKey =
  | "app.version"
  | "app.title"
  | "app.seed"
  | "button.restart"
  | "button.undo"
  | "button.shuffle"
  | "button.language"
  | "button.soundOn"
  | "button.soundOff"
  | "button.theme"
  | "status.hero"
  | "status.floor"
  | "status.level"
  | "status.hp"
  | "status.atk"
  | "status.def"
  | "status.gold"
  | "status.keys"
  | "status.weapon"
  | "status.shield"
  | "status.yellowKey"
  | "status.blueKey"
  | "status.redKey"
  | "wish.label"
  | "fight.title"
  | "fight.none"
  | "fight.canWin"
  | "fight.danger"
  | "fight.loss"
  | "fight.reward"
  | "book.title"
  | "book.empty"
  | "shop.title"
  | "shop.cost"
  | "log.title"
  | "controls.hint"
  | "overlay.victory"
  | "overlay.victorySub"
  | "overlay.fallen"
  | "overlay.fallenSub"
  | "result.victory"
  | "result.victorySub"
  | "result.fallen"
  | "result.fallenSub"
  | "result.moves"
  | "result.defeated"
  | "result.doors"
  | "result.shops"
  | "result.close"
  | `floor.${string}.title`
  | `floor.${string}.objective`
  | `monster.${MonsterKind}`
  | `item.${ItemKind}.name`
  | `item.${ItemKind}.description`
  | `shop.${ShopUpgrade}.label`
  | `shop.${ShopUpgrade}.description`
  | `log.${string}`;

export const LANG_STORAGE_KEY = "opd.language";
export const THEME_STORAGE_KEY = "opd.theme";

const dictionary: Record<Language, Record<string, string>> = {
  en: {
    "app.version": "v0.9 / Generator identity pass",
    "app.title": "OnePromptDungeon",
    "app.seed": "Seed",
    "button.restart": "Restart",
    "button.undo": "Undo Z",
    "button.shuffle": "Shuffle Text",
    "button.language": "中文",
    "button.soundOn": "SFX",
    "button.soundOff": "Mute",
    "button.theme": "Theme",
    "status.hero": "Hero",
    "status.floor": "Floor",
    "status.level": "Level",
    "status.hp": "HP",
    "status.atk": "ATK",
    "status.def": "DEF",
    "status.gold": "Gold",
    "status.keys": "Keys",
    "status.weapon": "Weapon",
    "status.shield": "Shield",
    "status.items": "Items",
    "status.yellowKey": "Yellow",
    "status.blueKey": "Blue",
    "status.redKey": "Red",
    "wish.label": "Tower wish",
    "objective.title": "Objective",
    "monsterForecast.title": "Monster forecast",
    "forecast.chance": "Chance to win",
    "map.title": "Floor map",
    "legend.you": "You",
    "legend.stairs": "Stairs",
    "legend.shop": "Shop",
    "legend.enemy": "Enemy",
    "legend.princess": "Princess",
    "legend.key": "Key",
    "legend.door": "Door",
    "legend.treasure": "Treasure",
    "route.title": "Route hint",
    "scanner.title": "Tower scanner",
    "scanner.reachable": "Reach",
    "scanner.pressure": "Pressure",
    "scanner.dependency": "Route",
    "scanner.safeFights": "Safe fights",
    "scanner.keys": "Keys Y/B/R",
    "scanner.doors": "Doors Y/B/R",
    "scanner.next": "Next",
    "scanner.solvable": "Solvable",
    "scanner.yes": "Yes",
    "scanner.no": "No",
    "scanner.next.stairs": "Reach stairs",
    "scanner.next.key": "Claim key",
    "scanner.next.safeFight": "Take safe fight",
    "scanner.next.shop": "Buy upgrade",
    "scanner.next.needKey": "Find a key",
    "scanner.next.reward": "Gather reward",
    "scanner.next.weapon": "Take weapon",
    "scanner.next.shield": "Take shield",
    "route.hero": "Hero",
    "route.collectKey": "Key",
    "route.openGate": "Gate",
    "route.reachStairs": "Stairs",
    "route.safeFight": "Safe fight",
    "route.takeReward": "Reward",
    "route.defeatBoss": "Boss",
    "route.rescuePrincess": "Princess",
    "route.rescued": "Rescued",
    "forge.title": "Wish Forge",
    "forge.wish": "Wish",
    "forge.seed": "Seed",
    "forge.difficulty": "Difficulty",
    "forge.generate": "Generate Tower",
    "forge.export": "Export JSON",
    "forge.share": "Share Link",
    "forge.local": "Local Generator",
    "preset.keyPuzzle": "Key Puzzle",
    "preset.bossRush": "Boss Rush",
    "preset.treasure": "Treasure",
    "preset.shop": "Shop Route",
    "report.title": "Solvability report",
    "report.solvable": "Solvable",
    "report.shape": "Shape",
    "report.pressure": "Pressure",
    "history.title": "Recent seeds",
    "history.empty": "No generated seeds yet.",
    "gallery.title": "Seed gallery",
    "gallery.beginner.name": "Beginner Tower",
    "gallery.keyPressure.name": "Key Starvation",
    "gallery.bossRush.name": "Boss Rush",
    "gallery.merchant.name": "Merchant Economy",
    "gallery.sword.name": "Sword First",
    "gallery.shield.name": "Shield First",
    "gallery.treasureTrap.name": "Treasure Trap",
    "gallery.oneHp.name": "One HP Escape",
    "style.shop": "Shop route",
    "style.keyPuzzle": "Key puzzle",
    "style.bossRush": "Boss rush",
    "style.treasure": "Treasure",
    "style.combat": "Combat",
    "style.equipment": "Equipment",
    "identity.balanced": "Balanced route",
    "identity.keyPressure": "Key pressure",
    "identity.bossRush": "Boss route",
    "identity.merchant": "Merchant route",
    "identity.weapon": "Weapon route",
    "identity.shield": "Shield route",
    "identity.treasure": "Treasure route",
    "identity.equipment": "Equipment route",
    "identity.combat": "Combat route",
    "equipment.none": "None",
    "equipment.ironSword": "Iron Sword",
    "equipment.silverSword": "Silver Sword",
    "equipment.ironShield": "Iron Shield",
    "equipment.silverShield": "Silver Shield",
    "difficulty.easy": "Easy",
    "difficulty.normal": "Normal",
    "difficulty.hard": "Hard",
    "generator.ready": "Ready",
    "generator.generated": "Generated",
    "generator.restarted": "Restarted",
    "generator.seedReady": "Seed ready",
    "generator.presetReady": "Preset ready",
    "generator.galleryLoaded": "Gallery seed loaded",
    "generator.restored": "Seed restored",
    "generator.shareCopied": "Share link copied",
    "generator.exportCopied": "Tower JSON copied",
    "fight.title": "Battle forecast",
    "fight.none": "No monster beside you.",
    "fight.canWin": "Can win",
    "fight.danger": "Do not fight",
    "fight.loss": "Loss",
    "fight.reward": "Reward",
    "book.title": "Monster Book",
    "book.empty": "No monsters remain on this floor.",
    "shop.title": "Merchant",
    "shop.cost": "20 gold each",
    "log.title": "Tower log",
    "controls.hint": "Move with WASD or arrow keys.",
    "overlay.victory": "VICTORY",
    "overlay.victorySub": "The princess is free.",
    "overlay.fallen": "FALLEN",
    "overlay.fallenSub": "Undo and choose another route.",
    "result.victory": "Tower Cleared",
    "result.victorySub": "The princess is free. Share this seed or rerun it with a tighter route.",
    "result.fallen": "Run Failed",
    "result.fallenSub": "The tower is still solvable. Undo or restart with what you learned.",
    "result.moves": "Moves",
    "result.defeated": "Defeated",
    "result.doors": "Doors",
    "result.shops": "Shops",
    "result.close": "Close",
    "floor.floor-1.title": "1F Stone Gate",
    "floor.floor-1.objective": "Learn the tower route: collect keys, gems, and reach the upper stairs.",
    "floor.floor-2.title": "2F Merchant Hall",
    "floor.floor-2.objective": "Spend gold wisely, open the moon door, and climb toward the sealed crown.",
    "floor.floor-3.title": "3F Armory Gate",
    "floor.floor-3.objective": "Claim the first weapon or shield before the upper guard line.",
    "floor.floor-4.title": "4F Split Forge",
    "floor.floor-4.objective": "Choose the upgrade route, manage red doors, and reach the final stairs.",
    "floor.floor-5.title": "5F Crystal Crown",
    "floor.floor-5.objective": "Defeat the Crystal Warden and rescue the princess.",
    "monster.greenSlime": "Green Slime",
    "monster.nightBat": "Night Bat",
    "monster.boneGuard": "Bone Guard",
    "monster.runeMage": "Rune Mage",
    "monster.ironKnight": "Iron Knight",
    "monster.towerWarden": "Crystal Warden",
    "item.smallPotion.name": "Red Potion",
    "item.smallPotion.description": "+160 HP",
    "item.largePotion.name": "Royal Potion",
    "item.largePotion.description": "+300 HP",
    "item.redGem.name": "Ruby",
    "item.redGem.description": "+6 ATK",
    "item.blueGem.name": "Sapphire",
    "item.blueGem.description": "+6 DEF",
    "item.yellowKey.name": "Yellow Key",
    "item.yellowKey.description": "Opens yellow doors",
    "item.blueKey.name": "Blue Key",
    "item.blueKey.description": "Opens blue doors",
    "item.redKey.name": "Red Key",
    "item.redKey.description": "Opens red doors",
    "item.ironSword.name": "Iron Sword",
    "item.ironSword.description": "+18 ATK",
    "item.silverSword.name": "Silver Sword",
    "item.silverSword.description": "+32 ATK",
    "item.ironShield.name": "Iron Shield",
    "item.ironShield.description": "+18 DEF",
    "item.silverShield.name": "Silver Shield",
    "item.silverShield.description": "+32 DEF",
    "shop.atk.label": "+12 ATK",
    "shop.atk.description": "Sharpen the hero's blade.",
    "shop.def.label": "+12 DEF",
    "shop.def.description": "Reforge the tower shield.",
    "shop.hp.label": "+250 HP",
    "shop.hp.description": "Drink a sealed royal elixir.",
    "log.initialWish": "The old tower accepts your wish and locks the crown room.",
    "log.generated": "Wish Forge shaped seed {seed} on {difficulty}.",
    "log.initialRoute": "Route carefully. Every key, fight, and shop visit matters.",
    "log.outerWall": "The outer wall has no door.",
    "log.wall": "Stone blocks the route.",
    "log.yellowDoorBlocked": "A yellow door waits for a yellow key.",
    "log.blueDoorBlocked": "A blue door waits for a blue key.",
    "log.redDoorBlocked": "A red door waits for a red key.",
    "log.yellowDoorOpened": "A yellow key turns. The door opens.",
    "log.blueDoorOpened": "A blue key hums. The door opens.",
    "log.redDoorOpened": "A red key burns bright. The door opens.",
    "log.monsterTooStrong": "{monster} is too strong. Expected loss: {loss} HP.",
    "log.defeated": "Defeated {monster}. Lost {loss} HP, gained {gold} gold and {exp} EXP.",
    "log.defeatedLevel": "Defeated {monster}. Lost {loss} HP, gained {gold} gold and {exp} EXP. Level {level}!",
    "log.claimed": "Claimed {item}: {description}.",
    "log.climbed": "Climbed to {floor}.",
    "log.descended": "Returned to {floor}.",
    "log.stairsUpBlocked": "The upper stairs are sealed.",
    "log.stairsDownBlocked": "The lower stairs have vanished.",
    "log.shopHere": "The merchant offers power for 20 gold.",
    "log.princessSealed": "The princess remains sealed until the Crystal Warden falls.",
    "log.victory": "The princess steps free. The tower opens to morning.",
    "log.fallen": "The hero falls inside the tower.",
    "log.noMerchant": "There is no merchant here.",
    "log.notEnoughGold": "Not enough gold. The merchant wants 20.",
    "log.buyAtk": "Bought sword training: +12 ATK.",
    "log.buyDef": "Bought shield training: +12 DEF.",
    "log.buyHp": "Bought a royal elixir: +250 HP.",
    "log.undoEmpty": "No previous step to undo.",
    "log.undid": "Undid the last step.",
  },
  zh: {
    "app.version": "v0.9 / 生成器识别度升级",
    "app.title": "OnePromptDungeon",
    "app.seed": "种子",
    "button.restart": "重开",
    "button.undo": "撤销 Z",
    "button.shuffle": "换一句",
    "button.language": "EN",
    "button.soundOn": "音效",
    "button.soundOff": "静音",
    "button.theme": "主题",
    "status.hero": "勇士",
    "status.floor": "楼层",
    "status.level": "等级",
    "status.hp": "生命",
    "status.atk": "攻击",
    "status.def": "防御",
    "status.gold": "金币",
    "status.keys": "钥匙",
    "status.weapon": "武器",
    "status.shield": "盾牌",
    "status.items": "道具",
    "status.yellowKey": "黄钥匙",
    "status.blueKey": "蓝钥匙",
    "status.redKey": "红钥匙",
    "wish.label": "塔之愿望",
    "objective.title": "目标",
    "monsterForecast.title": "怪物预估",
    "forecast.chance": "胜率",
    "map.title": "楼层地图",
    "legend.you": "你",
    "legend.stairs": "楼梯",
    "legend.shop": "商店",
    "legend.enemy": "敌人",
    "legend.princess": "公主",
    "legend.key": "钥匙",
    "legend.door": "门",
    "legend.treasure": "宝物",
    "route.title": "路线提示",
    "scanner.title": "魔塔扫描",
    "scanner.reachable": "可达",
    "scanner.pressure": "压力",
    "scanner.dependency": "路线",
    "scanner.safeFights": "安全战斗",
    "scanner.keys": "钥匙 黄/蓝/红",
    "scanner.doors": "门 黄/蓝/红",
    "scanner.next": "下一步",
    "scanner.solvable": "可通关",
    "scanner.yes": "是",
    "scanner.no": "否",
    "scanner.next.stairs": "前往楼梯",
    "scanner.next.key": "拿钥匙",
    "scanner.next.safeFight": "打安全怪",
    "scanner.next.shop": "购买属性",
    "scanner.next.needKey": "寻找钥匙",
    "scanner.next.reward": "收集奖励",
    "scanner.next.weapon": "拿武器",
    "scanner.next.shield": "拿盾牌",
    "route.hero": "勇士",
    "route.collectKey": "钥匙",
    "route.openGate": "开门",
    "route.reachStairs": "楼梯",
    "route.safeFight": "安全战斗",
    "route.takeReward": "奖励",
    "route.defeatBoss": "Boss",
    "route.rescuePrincess": "公主",
    "route.rescued": "已救出",
    "forge.title": "愿望熔炉",
    "forge.wish": "愿望",
    "forge.seed": "种子",
    "forge.difficulty": "难度",
    "forge.generate": "生成魔塔",
    "forge.export": "导出 JSON",
    "forge.share": "分享链接",
    "forge.local": "本地生成器",
    "preset.keyPuzzle": "钥匙谜题",
    "preset.bossRush": "Boss 速攻",
    "preset.treasure": "宝藏路线",
    "preset.shop": "商店路线",
    "report.title": "通关报告",
    "report.solvable": "可通关",
    "report.shape": "结构",
    "report.pressure": "压力",
    "history.title": "最近种子",
    "history.empty": "还没有生成记录。",
    "gallery.title": "精选种子",
    "gallery.beginner.name": "入门塔",
    "gallery.keyPressure.name": "钥匙饥荒",
    "gallery.bossRush.name": "Boss 速攻",
    "gallery.merchant.name": "商人经济",
    "gallery.sword.name": "先拿剑",
    "gallery.shield.name": "先拿盾",
    "gallery.treasureTrap.name": "宝藏陷阱",
    "gallery.oneHp.name": "一血逃生",
    "style.shop": "商店路线",
    "style.keyPuzzle": "钥匙谜题",
    "style.bossRush": "Boss 速攻",
    "style.treasure": "宝藏",
    "style.combat": "战斗",
    "style.equipment": "装备成长",
    "identity.balanced": "均衡路线",
    "identity.keyPressure": "钥匙压力",
    "identity.bossRush": "Boss 路线",
    "identity.merchant": "商人路线",
    "identity.weapon": "武器路线",
    "identity.shield": "盾牌路线",
    "identity.treasure": "宝藏路线",
    "identity.equipment": "装备路线",
    "identity.combat": "战斗路线",
    "equipment.none": "无",
    "equipment.ironSword": "铁剑",
    "equipment.silverSword": "银剑",
    "equipment.ironShield": "铁盾",
    "equipment.silverShield": "银盾",
    "difficulty.easy": "简单",
    "difficulty.normal": "普通",
    "difficulty.hard": "困难",
    "generator.ready": "就绪",
    "generator.generated": "已生成",
    "generator.restarted": "已重开",
    "generator.seedReady": "种子已准备",
    "generator.presetReady": "预设已准备",
    "generator.galleryLoaded": "精选种子已载入",
    "generator.restored": "种子已恢复",
    "generator.shareCopied": "分享链接已复制",
    "generator.exportCopied": "魔塔 JSON 已复制",
    "fight.title": "战斗预估",
    "fight.none": "身边没有怪物。",
    "fight.canWin": "可战",
    "fight.danger": "危险",
    "fight.loss": "损血",
    "fight.reward": "收益",
    "book.title": "怪物图鉴",
    "book.empty": "本层已经没有怪物。",
    "shop.title": "商人",
    "shop.cost": "每次 20 金币",
    "log.title": "塔内记录",
    "controls.hint": "使用 WASD 或方向键移动。",
    "overlay.victory": "胜利",
    "overlay.victorySub": "公主已经获救。",
    "overlay.fallen": "失败",
    "overlay.fallenSub": "撤销一步，换条路线。",
    "result.victory": "通关成功",
    "result.victorySub": "公主已经获救。可以分享这个种子，或尝试更优路线。",
    "result.fallen": "挑战失败",
    "result.fallenSub": "这座塔仍然可解。撤销一步或重开后重新规划。",
    "result.moves": "步数",
    "result.defeated": "击败",
    "result.doors": "开门",
    "result.shops": "交易",
    "result.close": "关闭",
    "floor.floor-1.title": "第 1 层 石门大厅",
    "floor.floor-1.objective": "学习路线规划：收集钥匙、宝石，找到上楼梯。",
    "floor.floor-2.title": "第 2 层 商人回廊",
    "floor.floor-2.objective": "合理花金币，打开蓝门，向塔顶前进。",
    "floor.floor-3.title": "第 3 层 武库之门",
    "floor.floor-3.objective": "在上层守卫线前拿到第一件武器或盾牌。",
    "floor.floor-4.title": "第 4 层 分岔熔炉",
    "floor.floor-4.objective": "选择升级路线，管理红门资源，抵达最终楼梯。",
    "floor.floor-5.title": "第 5 层 水晶王座",
    "floor.floor-5.objective": "击败水晶守卫，救出公主。",
    "monster.greenSlime": "绿色史莱姆",
    "monster.nightBat": "夜蝙蝠",
    "monster.boneGuard": "骷髅卫兵",
    "monster.runeMage": "符文法师",
    "monster.ironKnight": "铁甲骑士",
    "monster.towerWarden": "水晶守卫",
    "item.smallPotion.name": "红血瓶",
    "item.smallPotion.description": "+160 生命",
    "item.largePotion.name": "大血瓶",
    "item.largePotion.description": "+300 生命",
    "item.redGem.name": "红宝石",
    "item.redGem.description": "+6 攻击",
    "item.blueGem.name": "蓝宝石",
    "item.blueGem.description": "+6 防御",
    "item.yellowKey.name": "黄钥匙",
    "item.yellowKey.description": "打开黄门",
    "item.blueKey.name": "蓝钥匙",
    "item.blueKey.description": "打开蓝门",
    "item.redKey.name": "红钥匙",
    "item.redKey.description": "打开红门",
    "item.ironSword.name": "铁剑",
    "item.ironSword.description": "+18 攻击",
    "item.silverSword.name": "银剑",
    "item.silverSword.description": "+32 攻击",
    "item.ironShield.name": "铁盾",
    "item.ironShield.description": "+18 防御",
    "item.silverShield.name": "银盾",
    "item.silverShield.description": "+32 防御",
    "shop.atk.label": "+12 攻击",
    "shop.atk.description": "磨快勇士的剑。",
    "shop.def.label": "+12 防御",
    "shop.def.description": "重铸塔盾。",
    "shop.hp.label": "+250 生命",
    "shop.hp.description": "喝下皇家秘药。",
    "log.initialWish": "古塔回应了你的愿望，并封锁了王座之间。",
    "log.generated": "愿望熔炉用 {seed} 生成了{difficulty}魔塔。",
    "log.initialRoute": "谨慎规划。每把钥匙、每场战斗、每次交易都很重要。",
    "log.outerWall": "塔的外墙没有出口。",
    "log.wall": "石墙挡住了道路。",
    "log.yellowDoorBlocked": "黄门需要黄钥匙。",
    "log.blueDoorBlocked": "蓝门需要蓝钥匙。",
    "log.redDoorBlocked": "红门需要红钥匙。",
    "log.yellowDoorOpened": "黄钥匙转动，门打开了。",
    "log.blueDoorOpened": "蓝钥匙发光，门打开了。",
    "log.redDoorOpened": "红钥匙燃起微光，门打开了。",
    "log.monsterTooStrong": "{monster} 现在太强。预计损血：{loss}。",
    "log.defeated": "击败 {monster}。损失 {loss} 生命，获得 {gold} 金币和 {exp} 经验。",
    "log.defeatedLevel": "击败 {monster}。损失 {loss} 生命，获得 {gold} 金币和 {exp} 经验。升到 {level} 级！",
    "log.claimed": "获得 {item}：{description}。",
    "log.climbed": "上楼到 {floor}。",
    "log.descended": "下楼回到 {floor}。",
    "log.stairsUpBlocked": "上楼梯被封住了。",
    "log.stairsDownBlocked": "下楼梯消失了。",
    "log.shopHere": "商人愿意用 20 金币换取力量。",
    "log.princessSealed": "公主仍被封印，必须先击败水晶守卫。",
    "log.victory": "公主走出封印，魔塔迎来晨光。",
    "log.fallen": "勇士倒在了塔中。",
    "log.noMerchant": "这里没有商人。",
    "log.notEnoughGold": "金币不足，商人需要 20 金币。",
    "log.buyAtk": "购买剑术训练：攻击 +12。",
    "log.buyDef": "购买盾术训练：防御 +12。",
    "log.buyHp": "购买皇家秘药：生命 +250。",
    "log.undoEmpty": "没有可以撤销的步骤。",
    "log.undid": "已撤销上一步。",
  },
};

export function detectLanguage(): Language {
  if (typeof navigator !== "undefined" && navigator.language.toLowerCase().startsWith("zh")) {
    return "zh";
  }
  return "en";
}

export function translate(language: Language, key: TranslationKey | string, params: Record<string, string | number> = {}) {
  const template = dictionary[language][key] ?? dictionary.en[key] ?? key;
  return Object.entries(params).reduce((text, [name, value]) => text.replaceAll(`{${name}}`, String(value)), template);
}
